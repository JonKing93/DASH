...
% REVISED as per reviews in CPD... April 2013
%
% Given calibration-interval temperature, precipitation, and ring-width data,
% and site latitude, vslite_bayes_param_cal.m performes a Bayesian parameter
...
% used as priors.  The scheme also assumes independent, Gaussian model errors,
% and the amplitude of the error variance may also be estimated.
%
% Basic Usage:
% Basic: [T1,T2,M1,M2] = vslite_bayes_param_cal(T,P,phi,RW)
%
% Basic Inputs:
...
% RW: standardized annual calibration-interval ring-width.
%
% Basic Ouptuts:
...
%     'gparscalint'   Index of years to use to estimate the growth response
%                     parameters T1, T2, M1, M2. Default is all years.
%     'eparscalint'   Index of years to use to estimate the parameters of the
%                     error model if these parameters are to be estimated.
%                     Must be contiguous if using AR(1) error model. Default
%                     is all years (may underestimate error if not disjoint
...
%                     posterior draw with the maximum data likelihood or
%                     the posterior median as the point estimate for the
%                     growth reseponse parameter estimates and the error
%                     model parameter estimates.  Default is 'mle'.
%     'hydroclim'     Is the hydroclimate input variable (2nd input) precipitation ['P']
%                     or soil moisture ['M']? Default is ['P']; CPC Leaky
%                     Bucket model is then used to estimate M from input T
%                     and input P.
...
%                     Default is [1000].
%     'nbi'           Number of burn-in samples. Default is round(200).
%     'nchain'        3<=integer<=number of comp threads for the computation
%                     of Rhat. Default is 3.
%     'gparpriors'    growth parameter priors.  Either can be 'fourbet' for a
%                     set of informative four-parameter beta-distributed priors,
%                     or can be 'uniform'. In either case parameters can be
%                     specified by user, or else a default set will be used
%                     (see the following 5 property/value pairs for more
%                     information).
%                     see Tolwinski-Ward et al 2013, 'Bayesian parameter
%                     estimation and interpretation for an intermediate
%                     model of tree-ring width'  for details on literature-based
%                     parameters for the four-parameter beta distributed
%                     priors.
...
%                                1.5 2.8 0.0 0.1
%                                1.5 2.5 0.1 0.5]
%                     (See Tolwinski-Ward et al., 'Bayesian parameter estimation
%                     and interpretation for an intermediate model of tree-ring
%                     width', for justification of these choices.)
%     'convthresh'    scalar value greater than 0.  Threshold for MCMC
%                     convergence; warning is displayed if abs(Rhat-1)>convthresh.
%                     Default value is [1.1].
%     'ploutput'      logical 0/1; plot priors, posteriors, and estimates?
%                     Default is [0].
%     'verbose'       logical 0/1; print progress to screen? Default is [1].
%
...
%                 convergence, autocorrelation, multi-modality, or point estimates
%                 of parameters other than the median.
% Rhats:          Returns the convergence statistics associated with T1, T2, M1, M2,
%                 and sigma2rw if it was estimated.
% convwarning:    logical 0/1 depending on whether all Rhat values were outside of
%                 the threshold distance of 1.
% -- Next outputs for white noise error model (errormod==0): --
% sig2rw:         point estimate of model error variance
% sigma2rwdist:   returns the entire numerical posterior distribution
% -- Next outputs for AR(1) error model (errormod==1): --
% phi1:           Point estimate of AR(1) coefficient
% phi1dist:       Numerical distribution of AR(1) coefficient samples
% tau2:           Point estimate of error model innovations variance
% tau2dist:       Nunderical distribution of error model innovation
%                 variance samples
%
%
...
%                  model parameters also added; MLE added as option for point estimates;
...
%
% take zscore of RW data over interval used for fitting to fulfill assumptions of model error/noise
% structure
RW = zscore(RW);
%
...
if strcmp(hydroclim,'P') % if second input variable is precip,
    ...
    if substep == 1;
        [M,potEv,ndl,cdays] =...
            leakybucket_submonthly(1,Nyrs,phi,T,P,Mmax,Mmin,alpha,mth,muth,dr,Minit/dr);
    elseif substep == 0
        [M,potEv,ndl,cdays] =...
            leakybucket_monthly(1,Nyrs,phi,T,P,Mmax,Mmin,alpha,mth,muth,dr,Minit/dr);
    end
...
%
%%%% NOTE!!! Output graphics not yet revised to support estimates of error
%%%% model parameters. SETW 4/4/2013
if ploutput == 1 % if user requested plot of results:
    figure;
    %
    if strcmp(gparpriors,'uniform')
        set(gcf,'position',[680 320 560 775]);
        subplot(321);
        plot(aT1:((bT1-aT1)/100):bT1,(1/(bT1-aT1))*ones(1,101)); hold on;
        [f,xi] = ksdensity(Ttensemb);
        plot(xi,f,'r');
        %    legend('prior','psterior');
        title('T1','fontsize',14); xlim([aT1 bT1])
        %
        subplot(322);
        plot(aT2:((bT2-aT2)/100):bT2,(1/(bT2-aT2))*ones(1,101)); hold on;
        [f,xi] = ksdensity(Toensemb);
        plot(xi,f,'r');
        %     legend('prior','psterior');
        title('T2','fontsize',14); xlim([aT2 bT2])
        %%%%%%%%%%%%%%%%%%%%%
        %figure;
        %
        nbins = 100;
        subplot(323);
        plot(aM1:((bM1-aM1)/100):bM1,(1/(bM1-aM1))*ones(1,101)); hold on;
        [f,xi] = ksdensity(Mtensemb);
        plot(xi,f,'r');
        %      legend('prior','psterior');
        title('M1','fontsize',14); xlim([aM1 bM1])
        %
        subplot(324);
        plot(aM2:((bM2-aM2)/100):bM2,(1/(bM2-aM2))*ones(1,101)); hold on;
        [f,xi] = ksdensity(Moensemb);
        plot(xi,f,'r');
        %       legend('prior','psterior');
        title('M2','fontsize',14); xlim([aM2 bM2])
        %%%%%%%%%%%%%%%%%%%%%
        %figure;
        subplot(313)
        %
        rlim = wv2/(wv1+1)+3*sqrt(wv2^2/((wv1-2)*(wv1-1)^2));  % 3 standard deviations to the right of the prior mode
        xi = 0:rlim/300:rlim;
        f = ((wv2^wv1)/gamma(wv1))*(xi.^(-wv1-1)).*exp(-wv2./xi);
        plot(xi,f);
        hold on
        [f,xi] = ksdensity(sig2rwensemb);
        plot(xi,f,'r');
        %        legend('prior','posterior');
        title('\sigma^2_W','fontsize',14); xlim([0 rlim])
        %
        suptitle('Prior (blue) and posterior (red) densities of VS-Lite parameters')
    elseif strcmp(gparpriors,'fourbet')
        set(gcf,'position',[680 320 560 775]);
        subplot(321);
        % plot prior:
        xj = 0:.01:1;
        for j = 1:length(xj)
            T1prior(j) = betapdf(xj(j),aT1,bT1);
        end
        T1prior = T1prior/slpT1;
        plot(xj*slpT1+intT1,T1prior); hold on
        % plot smoothed histogram of posterior draws:
        [f,xi] = ksdensity(Ttensemb);
        plot(xi,f,'r');
        %    legend('prior','psterior');
        title('T1','fontsize',14); xlim([intT1 slpT1+intT1])
        %
        subplot(322);
        % plot prior:
        xj = 0:.01:1;
        for j = 1:length(xj)
            T2prior(j) = betapdf(xj(j),aT2,bT2);
        end
        T2prior = T2prior/slpT2;
        plot(xj*slpT2+intT2,T2prior); hold on
        % plot smoothed histogram of posterior draws:
        [f,xi] = ksdensity(Toensemb);
        plot(xi,f,'r');
        %     legend('prior','psterior');
        title('T2','fontsize',14); xlim([intT2 slpT2+intT2])
        %
        subplot(323);
        % plot prior:
        xj = 0:.01:1;
        for j = 1:length(xj)
            M1prior(j) = betapdf(xj(j),aM1,bM1);
        end
        M1prior = M1prior/slpM1;
        plot(xj*slpM1+intM1,M1prior); hold on
        % plot smoothed histogram of posterior draws:
        [f,xi] = ksdensity(Mtensemb);
        plot(xi,f,'r');
        %      legend('prior','psterior');
        title('M1','fontsize',14); xlim([intM1 slpM1+intM1])
        %
        subplot(324);
        % plot prior:
        xj = 0:.01:1;
        for j = 1:length(xj)
            M2prior(j) = betapdf(xj(j),aM2,bM2);
        end
        M2prior = M2prior/slpM2;
        plot(xj*slpM2+intM2,M2prior); hold on
        % plot smoothed histogram of posterior draws:
        [f,xi] = ksdensity(Toensemb);
        plot(xi,f,'r');
        %     legend('prior','psterior');
        ...
        plot(xi,f,'r');
        %       legend('prior','psterior');
        title('M2','fontsize',14); xlim([intM2 slpM2+intM2])
        %%%%%%%%%%%%%%%%%%%%%
        subplot(313)
        %
        rlim = wv2/(wv1+1)+3*sqrt(wv2^2/((wv1-2)*(wv1-1)^2));  % 3 standard deviations to the right of the prior mode
...
for i = 1:m
    ...
    Xbar(i) = nanmean(X(Nbi+1:end));
    % within-chain variances of X:
    Xs2(i) = nanvar(X(Nbi+1:end));
    allX = [allX X(Nbi+1:end)];
end
...
%
covhatXs2Xbar2 =...
    sum((Xs2-nanmean(Xs2)).*(Xbar.^2-nanmean(Xs2.^2)))/m; % ???? Check this!
covhatXs2Xbar =...
    sum((Xs2-nanmean(Xs2)).*(Xbar-nanmean(Xs2)))/m; % ???? Check this!
%
