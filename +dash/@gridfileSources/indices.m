...
try
    if isnumeric(sources) || islogical(sources)
        ...
        indices = dash.assert.indices(sources, nSources, 'sources', logicalRequirement, linearMax, header);
...
ME = MException(id, ['Input data source %.f ("%s") matches multiple data sources (sources %.f and %.f) ',...
...
ME = MException(id, 'No data sources match input data source %.f ("%s").\n\ngridfile: %s', ...
