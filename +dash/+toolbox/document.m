function[] = document(rebuildReference)
%% dash.toolbox.document  Builds the documentation for the DASH toolbox
% ----------
%   dash.toolbox.document
%   Builds the documentation for the DASH toolbox. Requires that the DASH
...
%   markup (using the config and rst files in doc_source/sphinx/resources)
%   to generate the final HTML reference set. The final set is located in
%   doc_source/sphinx/build/html.
%
%   dash.toolbox.document(rebuildReference)
...
%
% <a href="matlab:dash.doc('dash.toolbox.document')">Documentation Page</a>

%%%%% Parameters

% File locations (relative to the location of the doc_source folder)
...
% Get the location of the doc_source folder
fullpath = mfilename('fullpath');
folders = split(fullpath, filesep);
folders = [folders(1:end-3); 'doc_source'];
docSource = strjoin(folders, filesep);

% Locate the source folder
source = fullfile(docSource, rstDestination);

% Move to "doc_source", but return to current location when finished
currentDir = pwd;
