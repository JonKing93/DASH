function[meta] = buildMetadata( dimOrder, gridSize, atts, varargin)
...
% dimensions.
%
% meta = buildMetadata( dimOrder, gridSize, atts, dim1, meta1, dim2, meta2, ..., dimN, metaN )
% Converts user specified metadata to the metadata structure used by dash.
%
% ----- Inputs -----
%
% dimOrder: A cell of dimension IDs indicating the order of dimensions in
%       the gridded data source. A vector of strings. (nDim x 1)
%
% gridSize: The size of the dimensions in the gridded dataset. (nDim x 1)
%
% atts: A struct that contains non-dimensional metadata for the variable.
...
[dimOrder, metaDim] = setup( gridSize, dimOrder, atts, varargin );
...
for d = 1:nDim
    ...
    [isGridDim, g] = ismember( dimID(d), dimOrder);
    ...
    if ismeta
        ...
        value = gridFile.checkMetadata( value, dimSize, dimID(d) );
...
function[gridDims, metaDim] = setup( gridSize, gridDims, atts, meta )

% Check that the attributes are a scalar structure
if ~isstruct(atts) || ~isscalar( atts )
    error('atts must be a scalar structure.');
end
