...
for s = 1:numel(sources)
    if any(sources{s}.mergedSize ~= siz{s})
        ...
        error('The size of the %s dimension in %s (%.f) does not match the size recorded in .grid file %s (%.f).', ...
            sources{s}.mergedDims(bad), sources{s}.file, sources{s}.mergedSize(bad), obj.file, siz{s}(bad) );
    
    % And also that the data types match
    elseif ~strcmp(sources{s}.dataType, dataType(s))
        error('The data type of the %s variable in file %s (%s), does not match the data type recorded in file %s (%s)', ...
            sources{s}.var, sources{s}.file, sources{s}.dataType, obj.file, dataType(s));
    end
end
