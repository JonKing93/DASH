function[A, meta, dimID] = regrid( A, var, ensMeta, keepSingleton )
%% Regrids a variable from a particular time step for a DA analysis.
%
% [rA, meta, dimID] = dash.regrid( A, var, ensMeta, design )
% Regrids a variable in an ensemble.
%
% [rA, meta, dimID] = dash.regrid( A, var, ensMeta, design, keepSingle )
...
elseif size(A,1) ~= ensMeta.varIndices(end)
...
elseif ~isscalar(keepSingelton) || ~islogical(keepSingleton)
    error('keepSingleton must be a scalar logical.');
end
v = ensMeta.varCheck(var);
