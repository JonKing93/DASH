...
% CONSTRUCTOR:
% 
% obj = vslitePSM( obCoord, T1, T2, M1, M2, timeDA )
...
classdef vslitePSM < PSM
    ...
    properties
        timeDA; % The time steps processed by the DA
        lat;
        lon;
        T1;
        T2;
    ...
    methods
        ...
        % Constructor
        function obj = vslitePSM( obCoord, T1, T2, M1, M2, timeDA, varargin )
        ...
        % Run the PSM
        function[trw] = runPSM( obj, M, ~, ~, t )
            
            % Get the year of the integration
            vsYear = year( obj.timeDA(t) );
                        
            % Split the state vector elements into T and P
            T = M(1:12);
            P = M(13:24);
            ...
            % Run VS-Lite
            trw = VSLite_v2_5( vsYear, vsYear, obj.lat, obj.T1, obj.T2, obj.M1, obj.M2, ...
